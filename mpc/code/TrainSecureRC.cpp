  ...
  if (Param::NETWORK_TYPE == 0) {
    for (int l = 0; l < Param::N_HIDDEN + 1; l++) {
      ...
      if (Param::N_HIDDEN == 0 && l >= 1) {
        ...
        Init(W_layer, Param::FILTER_SIZE, 16);
        Init(b_layer, 16);
        ...
        if (Param::CNN_PADDING == "valid")
          Init(W_layer, 256, Param::N_NEURONS);
        else
          Init(W_layer, 256, Param::N_NEURONS);
  ...
  if (Param::NETWORK_TYPE == 0) {
    ...
    for (int l = 0; l < Param::N_HIDDEN; l++) {
      ...
      if (l == 0) {
        ...
        initial_reshape(X_reshape, X, 1, Param::BATCH_SIZE);
...
//          y[i][j] -= DoubleToFP(1, Param::NBIT_K, Param::NBIT_F);
  ...
  } else if (Param::LOSS == "softmax") {
    MatrixXm softmax_score;
    mpc.SoftMax(softmax_score, scores);
    dscores = softmax_score - y;
  } else {
...
//  if (Param::LOSS == "softmax") {
//    mse = dscores;
//  } else if (Param::LOSS == "mse") {
