...
bool read_matrix(Mat<ZZ_p> &matrix, ifstream &ifs, string fname, size_t n_rows,
                 size_t n_cols, MPCEnv &mpc) {
...
bool text_to_matrix(Mat<ZZ_p> &matrix, ifstream &ifs, string fname,
...
void AveragePool(Mat<ZZ_p> &avgpool, Mat<ZZ_p> &input, int kernel_size,
                 int stride) {
...
void initialize_parameters(Mat<ZZ_p> &W_layer, Vec<ZZ_p> &b_layer) {
  ...
  if (Param::DEBUG)
    tcout() << "W layer row, cols (" << W_layer.NumRows() << ", "
            << W_layer.NumCols() << ")" << endl;
...
void initialize_model(vector<Mat<ZZ_p>> &W, vector<Vec<ZZ_p>> &b,
                      ...
                      vector<Mat<ZZ_p>> &mW, vector<Vec<ZZ_p>> &mb, int pid,
                      MPCEnv &mpc) {
  ...
  for (int l = 0; l < Param::N_HIDDEN + 1; l++) {
    ...
    if (pid == 2) {
      ...
      //          DoubleToFP(W_layer[i][j], noise, Param::NBIT_K,
      //          Param::NBIT_F);
      //        }
      //      }
      ...
      if (Param::CACHED_PARAM_BATCH >= 0 && Param::CACHED_PARAM_EPOCH >= 0) {
        if (!text_to_matrix(W_layer, ifs,
                            Param::CACHE_FOLDER + "/ecg_P1_" +
                                to_string(Param::CACHED_PARAM_EPOCH) + "_" +
                                to_string(Param::CACHED_PARAM_BATCH) + "_W" +
                                to_string(l) + ".bin",
                            W_layer.NumRows(), W_layer.NumCols()))
          return;
        if (!text_to_vector(b_layer, ifs,
                            Param::CACHE_FOLDER + "/ecg_P1_" +
                                to_string(Param::CACHED_PARAM_EPOCH) + "_" +
                                to_string(Param::CACHED_PARAM_BATCH) + "_b" +
                                to_string(l) + ".bin"))
          return;
      } else {
...
double gradient_descent(Mat<ZZ_p> &X, Mat<ZZ_p> &y, vector<Mat<ZZ_p>> &W,
                        vector<Vec<ZZ_p>> &b, vector<Mat<ZZ_p>> &dW,
                        vector<Vec<ZZ_p>> &db, vector<Mat<ZZ_p>> &vW,
                        vector<Vec<ZZ_p>> &vb, vector<Mat<ZZ_p>> &mW,
                        vector<Vec<ZZ_p>> &mb, vector<Mat<ZZ_p>> &act,
                        vector<Mat<ZZ_p>> &relus, int epoch, int step, int pid,
                        MPCEnv &mpc) {
  //  if (pid == 2)
  ...
  for (int l = 0; l < Param::N_HIDDEN; l++) {
    ...
    if (l == 0) {
      ...
      if (Param::DEBUG)
        tcout() << "Reshape X to X_reshape : (" << X.NumRows() << ","
      ...
      if (Param::DEBUG)
        tcout() << "l = " << l << ", activation size " << act[l - 1].NumRows()
                << ", " << act[l - 1].NumCols() << endl;
  ...
  if (pid == 2) {
    if (Param::DEBUG)
      tcout() << "W.back() : (" << W.back().NumRows() << ", "
              << W.back().NumCols() << ")" << endl;
    if (Param::DEBUG)
      tcout() << "act.back() : (" << act.back().NumRows() << ", "
              << act.back().NumCols() << ")" << endl;
  }
...
//  exit(0);
  ...
  for (int l = Param::N_HIDDEN; l >= 0; l--) {
    ...
    if (pid == 2) {
      if (Param::DEBUG)
        tcout() << "X_T : (" << X_T.NumRows() << ", " << X_T.NumCols() << ")"
                << endl;
      if (Param::DEBUG)
        tcout() << "dhidden : (" << dhidden.NumRows() << ", "
    ...
    if (X_T.NumCols() != dhidden.NumRows()) {
      ...
      if (l == 3) {
        ...
        if (Param::DEBUG)
          tcout() << "X_T -> converted : (" << X_T.NumRows() << ", "
                  << X_T.NumCols() << ")" << endl;
    ...
    //      tcout() << "W[l] : " << W[l].NumRows() << "/" << W[l].NumCols() <<
    //      endl; tcout() << "dW[l] : " << dW[l].NumRows() << "/" <<
    //      dW[l].NumCols() << endl; tcout() << "reg : " << reg.NumRows() << "/"
    //      << reg.NumCols() << endl;
    //    }
    //    dW[l] += reg;
    ...
    if (l > 0) {
      ...
      if (pid == 2 && Param::DEBUG) {
        tcout() << "dhidden: " << dhidden.NumRows() << "/" << dhidden.NumCols()
                << endl;
...
void load_X_y(string suffix, Mat<ZZ_p> &X, Mat<ZZ_p> &y, int pid, MPCEnv &mpc) {
  if (pid == 0)
...
void model_update(Mat<ZZ_p> &X, Mat<ZZ_p> &y, vector<Mat<ZZ_p>> &W,
                  vector<Vec<ZZ_p>> &b, vector<Mat<ZZ_p>> &dW,
                  vector<Vec<ZZ_p>> &db, vector<Mat<ZZ_p>> &vW,
                  vector<Vec<ZZ_p>> &vb, vector<Mat<ZZ_p>> &mW,
                  vector<Vec<ZZ_p>> &mb, vector<Mat<ZZ_p>> &act,
                  vector<Mat<ZZ_p>> &relus, int &epoch, int pid, MPCEnv &mpc) {
  ...
  for (int i = 0; i < batches_in_file; i++) {
    ...
    if (epoch == Param::CACHED_PARAM_EPOCH && i < Param::CACHED_PARAM_BATCH) {
      if (Param::DEBUG)
        tcout() << "Epoch : " << epoch << " - Batch : " << i << " skipped"
                << endl;
      continue;
    }
    ...
    for (int j = base_j; j < base_j + Param::BATCH_SIZE && j < X.NumRows();
         j++) {
    ...
    if (Param::DEBUG)
      tcout() << "x = " << X_batch.NumRows() << ", " << X_batch.NumCols()
              << endl;
    ...
    if (i % Param::LOG_PER_BATCH == 0 && i > 0) {
      ...
      //            mpc.SwitchSeed(pn);
      mpc.ExportSeed(fs, pid);
      fs.close();
    }

    /* Do one round of mini-batch gradient descent. */
    double mse_score = gradient_descent(
    ...
    if (pid == 2) {
      ...
      tcout() << "epoch: " << epoch << " batch: " << i + 1 << "/"
              << batches_in_file << " loss : " << mse_score
              << " laptime : " << laptime << " total time: " << hour << ":"
              << minute << ":" << second << endl;
    }
...
bool dti_protocol(MPCEnv &mpc, int pid) {
  ...
  for (int epoch = 0; epoch < Param::MAX_EPOCHS;
    ...
    if (epoch < Param::CACHED_PARAM_EPOCH) {
      if (Param::DEBUG)
        tcout() << "Epoch : " << epoch << " skipped" << endl;
      epoch++;
      continue;
    }

    /* Do model updates and file reads in parallel. */
    model_update(X, y, W, b, dW, db, vW, vb, mW, mb, act, relus, epoch, pid,
                 mpc);
