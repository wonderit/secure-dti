...
/* Perform the actual reduction with unrolling */
        ...
        struct packetwise_redux_impl<Func, Evaluator, CompleteUnrolling> {
            ...
            PacketType run(const Evaluator &eval, const Func &func, Index /*size*/) {
              return redux_vec_unroller<Func, Evaluator, 0, packetwise_redux_traits<Func, Evaluator>::OuterSize>::template run<PacketType>(
                      eval, func);
            }
        };
...
/* Perform the actual reduction for dynamic sizes */
        ...
        enum {
            ...
            _Vectorizable = bool(int(_ArgFlags) & PacketAccessBit)
                            ...
                            &&
                            (Direction == int(Vertical) ? bool(_ArgFlags & RowMajorBit) : (_ArgFlags & RowMajorBit) ==
                                                                                          0)
                            && (TraversalSize != 0),
        ...
        PacketType packet(Index idx) const {
          ...
          PacketType p = internal::packetwise_redux_impl<BinaryOp, PanelEvaluator>::template run<PacketType>(panel_eval,
