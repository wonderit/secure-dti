...
//     OuterStrideAtCompileTime = Dynamic,
            ...
            FlagsLinearAccessBit = (traits<XprType>::RowsAtCompileTime == 1 ||
                                    traits<XprType>::ColsAtCompileTime == 1 || HasDirectAccess) ? LinearAccessBit : 0,
    ...
    struct reshaped_evaluator<ArgType, Rows, Cols, Order, /* HasDirectAccess */ false>
        ...
        EIGEN_DEVICE_FUNC explicit reshaped_evaluator(const XprType &xpr) : m_argImpl(xpr.nestedExpression()),
                                                                            m_xpr(xpr) {
          EIGEN_INTERNAL_CHECK_COST_VALUE(CoeffReadCost);
        }
