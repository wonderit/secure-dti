...
// Prototypes of non-user callable routines
        template<typename IndexType>
        static IndexType
        init_rows_cols(IndexType n_row, IndexType n_col, RowStructure<IndexType> Row[], ColStructure<IndexType> col[],
                       IndexType A[], IndexType p[], IndexType stats[NStats]);

        template<typename IndexType>
        static void
        init_scoring(IndexType n_row, IndexType n_col, RowStructure<IndexType> Row[], ColStructure<IndexType> Col[],
                     IndexType A[], IndexType head[], double knobs[NKnobs], IndexType *p_n_row2, IndexType *p_n_col2,
                     IndexType *p_max_deg);
        ...
        template<typename IndexType>
        static void
        detect_super_cols(ColStructure<IndexType> Col[], IndexType A[], IndexType head[], IndexType row_start,
 ...
 */
        ...
        static bool compute_ordering(IndexType n_row, IndexType n_col, IndexType Alen, IndexType *A, IndexType *p,
          ...
          {
            stats[Colamd::Status] = Colamd::ErrorANotPresent;COLAMD_DEBUG0 (("colamd: A not present\n"));
            return (false);
          }

          if (!p)   /* p is not present */
          {
            stats[Colamd::Status] = Colamd::ErrorPNotPresent;COLAMD_DEBUG0 (("colamd: p not present\n"));
            return (false);
          }
          ...
          if (n_row < 0)  /* n_row must be >= 0 */
          {
            ...
            stats[Colamd::Info1] = n_row;COLAMD_DEBUG0 (("colamd: nrow negative %d\n", n_row));
            return (false);
          }
          ...
          if (n_col < 0)  /* n_col must be >= 0 */
          {
            ...
            stats[Colamd::Info1] = n_col;COLAMD_DEBUG0 (("colamd: ncol negative %d\n", n_col));
            return (false);
          }
          ...
          if (p[0] != 0) {
            ...
            stats[Colamd::Info1] = p[0];COLAMD_DEBUG0 (("colamd: p[0] not zero %d\n", p[0]));
            return (false);
          }
          ...
          if (need > Alen) {
            ...
            stats[Colamd::Info2] = Alen;COLAMD_DEBUG0 (("colamd: Need Alen >= %d, given only Alen = %d\n", need, Alen));
            return (false);
          }
          ...
          stats[Colamd::DefragCount] = ngarbage;COLAMD_DEBUG0 (("colamd: done.\n"));
          return (true);
        }
        ...
        static IndexType init_rows_cols  /* returns true if OK, or false otherwise */
          ...
          for (col = 0; col < n_col; col++) {
            ...
            if ((Col[col].length) < 0) // extra parentheses to work-around gcc bug 10200
            {
              ...
              stats[Colamd::Info2] = Col[col].length;COLAMD_DEBUG0 (
          ...
          for (col = 0; col < n_col; col++) {
            ...
            while (cp < cp_end) {
              ...
              if (row < 0 || row >= n_row) {
                ...
                stats[Colamd::Info3] = n_row;COLAMD_DEBUG0 (("colamd: row %d col %d out of bounds\n", row, col));
                return (false);
              }
              ...
              if (row <= last_row || Row[row].shared2.mark == col) {
                ...
                stats[Colamd::Info1] = col;
                stats[Colamd::Info2] = row;
                (stats[Colamd::Info3])++;COLAMD_DEBUG1 (("colamd: row %d col %d unsorted/duplicate\n", row, col));
              }
          ...
          if (stats[Status] == OkButJumbled) { COLAMD_DEBUG0 (("colamd: reconstructing column form, matrix jumbled\n"));
...
*/
        ...
        static void init_scoring
          ...
          dense_col_count = numext::maxi(IndexType(0), numext::mini(IndexType(knobs[Colamd::DenseCol] * n_row),
                                                                    n_row));COLAMD_DEBUG1 (
          ...
          for (c = n_col - 1; c >= 0; c--) {
            ...
            if (Col[c].is_alive()) { COLAMD_DEBUG4 (("place %d score %d minscore %d ncol %d\n",
...
*/
        ...
        static IndexType find_ordering /* return the number of garbage collections */
          ...
          for (k = 0; k < n_col2; /* 'k' is incremented below */) {
            ...
            COLAMD_ASSERT (Col[pivot_col].is_alive());COLAMD_DEBUG3 (("Pivot col: %d\n", pivot_col));
            ...
            while (rp < rp_end) {
              col = *rp++;
              COLAMD_ASSERT (Col[col].is_alive() && col != pivot_col);COLAMD_DEBUG3 (("Col: %d\n", col));
