    ...
    for l in range(N_HIDDEN):
        ...
        if l == 0:
            ...
            if l == 1 or l == 2:
                ...
                print('act shape', act[l-1].shape, 'w shape', W[l].shape)
                print('act_col, w_row', act_col, w_row)
                if act_col != w_row:
                    act[l-1] = act[l-1].reshape(act[l-1].shape[0], -1)
                    # W[l] = W[l][:336, :]
                print('act shape', act[l-1].shape)

            print('W shape : ', W[l].shape, 'b shape : ', b[l].shape)
...
def load_model():
    ...
    for l in range(N_HIDDEN+1):
        W[l] = np.loadtxt('mpc/cache/pytorch2/W{}_final.txt'.format(l))

    # Initialize bias vector with zeros.
    b = [ []  for _ in range(N_HIDDEN + 1) ]
    for l in range(N_HIDDEN+1):
        b[l] = np.loadtxt('mpc/cache/pytorch2/b{}_final.txt'.format(l))
        print('b sh', b[l].shape, b[l].size)
        print('b', b[l])
        # if b[l].size > 1:
        #     b[l] = b[l].reshape(b[l].shape[0], 1)
        print('b sh', b[l].shape)
        print('b', b[l])
        # exit()
        #     b reshape for torch
